...
def multiply_inputs(m1, m2):
    ...
    if np.array(m1).shape[1] != np.array(m2).shape[0] and np.array(m1).shape[0] != np.array(m2).shape[1]:
        return False

    # TODO: If you have not returned False, then calculate the matrix product
    #       of m1 and m2 and return it. Do not use a transpose,
    #       but you swap their order if necessary
    if np.array(m1).shape[1] == np.array(m2).shape[0]:
        return np.matmul(np.array(m1), np.array(m2))

    if np.array(m1).shape[0] == np.array(m2).shape[1]:
        return np.matmul(np.array(m2), np.array(m1))
